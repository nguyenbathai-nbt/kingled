...
class RestLoginController extends Controller
{
    protected function getLogin()
    {
        ...
        try {
            $check = $this->auth->checkSiteHome([
                'username' => trim($infomation['username']),
                'password' => trim($infomation['password']),
            ]);
            ...
            if ($check == 'true') {
                $list = [
                    ...
                    'name' => $auth_site_home['full_name'],
                    'id' => $auth_site_home['id'],
                    'avatar' => $user->image->getImageUrl()
                ];
            } else {
                $list = [
                    'code' => 2,
    ...
    protected function getSignUp()
    {
        ...
        $role_user_id = Role::getIdByCode('USER');
        $user = new Users();
        $list = [];
        $user->setId($user->getSequenceId());
        $user->setStatusId($status_id_enable);
        $user->setPassword($infomation['password']);
        $error_check_validitions = Users::checkValidations($infomation);
        if (count($error_check_validitions) != 0) {
            $list['code'] = 2;
            foreach ($error_check_validitions as $index => $message) {
                $list['message_error_' . $index] = $message;
            }
        } else {
            $token = openssl_random_pseudo_bytes(16);
            $token = bin2hex($token);
            $user->setUsername($infomation['username']);
            $user->setEmail($infomation['email']);
            $user->setPhone($infomation['phone']);
            ...
            if ($user->save()) {
                ...
                $list['code'] = 2;
                foreach ($user->getMessages() as $index => $message) {
                    $list['message_error_' . $index] = $message->getMessage();
                }
            }
        }
