...
use Subscriber\Modules\Session\Forms\LoginForm;
use Subscriber\Modules\Session\Forms\SignUpForm;
        ...
        if ($this->request->isPost()) {
            ...
            try {
                if ($form->isValid($this->request->getPost()) == false) {
                    ...
                    if ($check == 'true') {
                        if ($this->session->get('previous-url') != null) {
                            $previous_url = $this->session->get('previous-url');
                            $this->session->remove('previous-url');
                            return $this->response->redirect($previous_url);

                        } else {
                            return $this->response->redirect('/all-badge');
            ...
            {
                return $this->redirect('/all-badge');
            }
        }
        $form->clear();
        $this->view->form = $form;
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if ($post['re_password'] != $post['password']) {
                    ...
                    $user->setLastFailAttempt( date('Y-m-d G:i:s'));
                    $user->setLastLogin( date('Y-m-d G:i:s'));
                    $user->setLastPasswdUpdate( date('Y-m-d G:i:s'));
