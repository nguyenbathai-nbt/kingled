...
class ReportController extends DashboardControllerBase
{
    ...
    public function reportViaMonthAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $excel->getActiveSheet()->getColumnDimension('E')->setWidth(75);
            ...
            foreach ($bills as $bill) {
                ...
                $excel->getActiveSheet()->setCellValueByColumnAndRow(1, $current_line, mb_split('-', $bill->getName())[0]);
                $excel->getActiveSheet()->setCellValueByColumnAndRow(2, $current_line, (string)mb_split('-', $bill->getName())[1]);
                ...
                $excel->getActiveSheet()->setCellValueByColumnAndRow(3, $current_line, $bill_detail->product->getCode());
                $excel->getActiveSheet()->setCellValueByColumnAndRow(4, $current_line, $bill_detail->product->getName());
                $excel->getActiveSheet()->setCellValueByColumnAndRow(5, $current_line, $bill_detail->getQuantity());
                $total_quantity += $bill_detail->getQuantity();
                $excel->getActiveSheet()->setCellValueByColumnAndRow(6, $current_line, $bill->status->getName());
                ...
                foreach ($times as $time) {
                    if ($time->getMajorId() == 1) {
                        ...
                        $excel->getActiveSheet()->setCellValueByColumnAndRow(10, $current_line, floor($time->getCountTime() / 60) . ' phút ' . $time->getCountTime() % 60 . ' giây');
            ...
            $current_line += 1;
            $excel->getActiveSheet()->setCellValueByColumnAndRow(5, $current_line, $total_quantity);
            $excel->getActiveSheet()->setCellValueByColumnAndRow(7, $current_line, floor($total_6 / 60) . ' phút ' . $total_6 % 60 . ' giây');
            $excel->getActiveSheet()->setCellValueByColumnAndRow(8, $current_line, floor($total_7 / 60) . ' phút ' . $total_7 % 60 . ' giây');
            $excel->getActiveSheet()->setCellValueByColumnAndRow(9, $current_line, floor($total_8 / 60) . ' phút ' . $total_8 % 60 . ' giây');
            $excel->getActiveSheet()->setCellValueByColumnAndRow(10, $current_line, floor($total_9 / 60) . ' phút ' . $total_9 % 60 . ' giây');
            $transcriptStyle = ['borders' => ['allborders' => ['style' => PHPExcel_Style_Border::BORDER_THIN]]];
            $excel->getActiveSheet()->getStyle("A3:" . 'K' . $current_line)->applyFromArray($transcriptStyle);
